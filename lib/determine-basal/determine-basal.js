    ...
    if (profile.use_autoisf && profile.enableSMB_EvenOn_OddOff_always)  {
        ...
        if ( !evenTarget ){
            ...
            if (target < 100) {     // indirect asessment; later set it in GUI
                console.error("eff.iobTH: " + round(iobThEffective,1) + "IU, IOB% of iobTH at " + round(iob_data.iob/(profile.max_iob*iobThPercent)*100,2) + "%")
...
// Adjust ISF based on sensitivityRatio
    ...
    console.error(" iobTH from profile: " + profile.iob_threshold_percent + ", maxIOB: " + profile.max_iob + ", iobTH_RedRatio: " + iobTH_reduction_ratio);
...
// Print summary variables based on predBGs etc.
    ...
    rT.IOB = iob_data.iob;
    rT.iob_THeffective = iob_ThEffective;
