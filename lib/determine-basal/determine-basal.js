...
/*function autoisfOn(profile, autoisfAllowed) {
    if(profile.use_autoisf === false) {
	console.error("AutoISF disabled");
	return false;
    } else if (profile.use_autoisf === true) {
        console.error("autoISF is enabled");
        return true;
    } 
}
/*function use_autoisf(autoisf_on, profile) {
	if(profile.use_autoisf === true) {
		console.error("AutoISF enabled");
		return true;
	}
	console.error("AutoISF disabled in preferences");
	return false;

var useAutoISF = autoisfOn(profile);*/

var sensitivityRatio = 1;
//console.error("AutoISF setting in preferences"+profile.use_autoisf+"; ");
//console.error("Sensitivity Ratio ="+sensitivityRatio+"; "); 
//Start AutoISF function
function autoISF(sens, target_bg, profile, glucose_status, meal_data, autosens_data, sensitivityRatio)
{   if(typeof profile.autosisf_max === 'undefined') {
        console.error("AutoISF not enabled in preferences");
        return sens;
    } else if(profile.autoisf_max) {
                //var sensitivityRatio = autosens_data.ratio;
        //console.error("Sensitivity Ratio ="+sensitivityRatio+"; ");
        ...
        var weightISF = Math.min(profile.autoisf_hourlychange, 1);           // mod 7d: specify factor directly; use factor 0 to shut autoISF OFF
        if (meal_data.mealCOB==0 && dura05>=10) {
            if (avg05 > target_bg) {
                // # fight the resistance at high levels
                var maxISFReduction = Math.min(profile.autoisf_max, 2);      // mod 7d
                ...
                var ISFreductionLevel = Math.min(maxISFReduction, levelISF);
                console.error("ISFreductionLevel = " +ISFreductionLevel+" ;");
                var liftISF = Math.max(ISFreductionLevel, sensitivityRatio);  // corrected logic on 30.Jan.2021
                console.error("dura05_weight = "+dura05_weight+";  avg05_weight = "+avg05_weight+"; levelISF = "+levelISF+"; liftISF = "+liftISF+"; ");
	            ...
	            console.error("sens is now" +sens+"; ");
            } else {
        ...
        console.error("AutoISF has run. Sens is now "+sens+" ;");
        return sens;
    }
}
