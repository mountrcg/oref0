...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data, currentTime, pumphistory, preferences, basalprofile, tdd, tdd_averages) {
    ...
    var DynISFFormula = preferences.enableDynamicISF;
    ...
    if (DynISFFormula == true && tdd > 0) {
        ...
        // Set the new ratio
       // autosens_data.ratio = newRatio;   //switching it off
    ...
    if (DynISFFormula == false && useDynamicCR == false) {
        tddReason += "";
...
//    var sensitivityRatio;
    ...
    if (sensitivityRatio && profile.enableDynISF == false) { // Disable adjustment of basal by sensitivityRatio when using dISF
        basal = profile.current_basal * sensitivityRatio;
        basal = round_basal(basal, profile)
    }

    else if (profile.enableDynISF == true && profile.tddAdjBasal == true) {
