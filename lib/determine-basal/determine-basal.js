...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data, currentTime, pumphistory, preferences, basalprofile, tdd, tdd_averages) {
    ...
    // Dynamic ratios
    var DynISFFormula = profile.enableDynamicISF;
    if (DynISFFormula == true) {
        const BG = glucose_status.glucose;
        var useDynamicCR = profile.enableDynamicCR;
        const minLimitDynISF = profile.autosens_min;
        const maxLimitDynISF = profile.autosens_max;
        ...
        if (profile.use_autoisf == true && DynISFFormula == true) {
            profile.use_autoisf = false;
        }

        // Turn off DynISF' formula (and AutoISF) when using a temp target >= 118 (6.5 mol/l) and if an exercise setting is enabled.
        ...
        if (DynISFFormula == true && tdd > 0) {

            log_isfCR = ", Dynamic ISF/CR, On/";

            // Respect autosens.max and autosens.min limitLogs
            if (newRatio > maxLimitDynISF) {
                log = ", Dynamic ISF limited by autosens_max setting: " + maxLimitDynISF + " (" +  round(newRatio,2) + "), ";
                limitLog = ", Autosens/Dynamic Limit: " + maxLimitDynISF + " (" +  round(newRatio,2) + ")";
                newRatio = maxLimitDynISF;
            } else if (newRatio < minLimitDynISF) {
                log = ", Dynamic ISF limjted by autosens_min setting: " + minLimitDynISF + " (" +  round(newRatio,2) + "). ";
                limitLog = ", Autosens/Dynamic Limit: " + minLimitDynISF + " (" +  round(newRatio,2) + ")";
                newRatio = minLimitDynISF;
            }
            ...
            if (useDynamicCR == true) {
                ...
                var logCR = " CR: " + cr + " g/U";
                profile.carb_ratio = cr;  //switching it off
        ...
        } else if (DynISFFormula == false && useDynamicCR == true) {
...
//    var sensitivityRatio;
    ...
    if (sensitivityRatio && profile.enableDynamicISF == false) { // Disable adjustment of basal by sensitivityRatio when using dISF
        basal = profile.current_basal * sensitivityRatio;
        basal = round_basal(basal, profile)
    }

    else if (profile.enableDynamicISF == true && profile.tddAdjBasal == true) {
